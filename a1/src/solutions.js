...
function normalizeVariable(value) {
  ...
  trueValue = value.toUpperCase().trim();

  trueValue = trueValue.replaceAll('.', ' ').split(/\s+/).join('_');
...
function createIframe(src, height, allowFullScreen) {
  ...
  var iframe = `<iframe src="${trimmedSrc}"`;

  if (Number.isInteger(tempHeight) && tempHeight > 0) {
...
function fixPostalCode(postalCode) {
  var firstPart = '';
  var secondPart = '';
  var invalidFirst = 'DFIOQWUZ';
  var invalidRest = 'DFIOQU';
  var fixedPostal = '';
  var i;

  if (postalCode !== null) {
    fixedPostal = postalCode.replace(/\s/g, '');

    if (fixedPostal.length !== 6) {
      throw new Error('Parameter is an incorrect length!');
    }

    fixedPostal = fixedPostal.toUpperCase();

    if (invalidFirst.includes(fixedPostal[0])) {
      throw new Error('Illegal character used in first position of parameter!');
    }

    for (i = 1; i < fixedPostal.length; i++) {
      if (i % 2) {
        if (fixedPostal[i] < '0' || fixedPostal[i] > '9') {
          throw new Error(`Character ${i} of parameter is invalid!`);
        }
      } else {
        if (!fixedPostal[i].match(/[a-z]/i)) {
          throw new Error(`Illegal character used in position ${i} of parameter!`);
        }

        if (invalidRest.includes(fixedPostal[i])) {
          throw new Error(`Illegal character used in position ${i} of parameter!`);
        }
      }
    }

    firstPart = fixedPostal.substring(0, 3);
    secondPart = fixedPostal.substring(3, 7);

    fixedPostal = `${firstPart} ${secondPart}`;
...
function toProvince(postalCode, useShortForm) {
  ...
  try {
    fixedPostal = fixPostalCode(postalCode);
  } catch (error) {
    return null;
  }

  if (ontario.includes(fixedPostal[0])) {
    area = useShortForm ? 'ON' : 'Ontario';
  } else if (quebec.includes(fixedPostal[0])) {
    area = useShortForm ? 'QC' : 'Quebec';
  } else {
    switch (fixedPostal[0]) {
      case 'B':
        area = useShortForm ? 'NS' : 'Nova Scotia';
        break;
      case 'E':
        area = useShortForm ? 'NB' : 'New Brunswick';
        break;
      case 'R':
        area = useShortForm ? 'MB' : 'Manitoba';
        break;
      case 'V':
        area = useShortForm ? 'BC' : 'British Columbia';
        break;
      case 'C':
        area = useShortForm ? 'PE' : 'Prince Edward Island';
        break;
      case 'S':
        area = useShortForm ? 'SK' : 'Saskatchewan';
        break;
      case 'T':
        area = useShortForm ? 'AB' : 'Alberta';
        break;
      case 'A':
        area = useShortForm ? 'NL' : 'Newfoundland and Labrador';
        break;
      case 'X':
        area = useShortForm ? 'NT' : 'Northwest Territories and Nunavut';
        break;
      case 'Y':
        area = useShortForm ? 'YT' : 'Yukon';
        break;
      default:
        area = 'ERROR';
    }
...
function normalizeCoord(value) {
  ...
  trueValue = value.replaceAll(' ', '').replaceAll('[', '').replaceAll(']', '');

  i = trueValue.indexOf(',');

  firstPart = trueValue.substring(0, i).trim();
  secondPart = trueValue.replaceAll(',', '').substring(i).trim();

  if (reversed === true) {
    trueValue = firstPart;
    firstPart = secondPart;
    secondPart = trueValue;
  }

  if (Number(firstPart) > 90 || Number(firstPart) < -90) {
    throw new Error('Illegal latitude value passed as first parameter!');
  }

  if (Number(secondPart) > 180 || Number(secondPart) < -180) {
    throw new Error('Illegal longitude value passed as first parameter!');
  }

  trueValue = `(${firstPart}, ${secondPart})`;
...
function formatCoords(...values) {
  ...
  if (values !== null) {
    for (i = 0; i < values.length; i++) {
      ...
      if (values[i] !== null) {
        try {
          tempValue = normalizeCoord(values[i]);
        } catch (error) {
          errorOccured = true;
        }
...
function countForProvince(provinceCode, ...postalCodes) {
  ...
  if (postalCodes.length === 0) {
    throw 'No postal codes have been passed as the second parameter.';
  }

  for (i = 0; i < postalCodes.length; i++) {
    errorT = false;
    if (postalCodes[i] === null) {
      throw new Error(`Postal code at position ${i + 1} cannot be null!`);
    }

    if (typeof postalCodes[i] !== 'string') {
      throw new Error(`Postal code at position ${i + 1} is not a string!`);
    }

    try {
      shortPost = toProvince(postalCodes[i], true);
    } catch (error) {
      errorT = true;
    }
    ...
    if (
      errorT === false &&
      shortPost !== null &&
      fixPostalCode(postalCodes[i]) === postalCodes[i]
    ) {
      if (shortPost === provinceCode) {
        validCount++;
      }
    }
  }
...
function generateLicenseLink(licenseCode, targetBlank) {
  var licenseDictionary = {
    'CC-BY': ['by', 'Creative Commons Attribution License'],
    'CC-BY-NC': ['by-nc', 'Creative Commons Attribution-NonCommercial License'],
    'CC-BY-SA': ['by-sa', 'Creative Commons Attribution-ShareAlike License'],
    'CC-BY-ND': ['by-nd', 'Creative Commons Attribution-NoDerivs License'],
    'CC-BY-NC-SA': ['by-nc-sa', 'Creative Commons Attribution-NonCommercial-ShareAlike License'],
    'CC-BY-NC-ND': ['by-nc-nd', 'Creative Commons Attribution-NonCommercial-NoDerivs License']
  };
  ...
  var values = Object.entries(licenseDictionary);
  //var keys = Object.keys(licenseDictionary);

  var url = `<a href="https://creativecommons.org/licenses/`;
  ...
  if (foundValue === true) {
    ...
    url = '<a href="https://choosealicense.com/no-permission/">All Rights Reserved</a>';
  }
...
function pureBool(value) {
  ...
  if ((typeof value === 'boolean' && value === true) || value === false) {
    ...
    if (value[0].match(tRegex)) {
      bool = true;
    } else if (value[0].match(fRegex)) {
      bool = false;
    }
  } else {
...
function every(...array) {
  ...
  for (i = 0; i < array.length; i++) {
    try {
      tempArray[i] = pureBool(array[i]);
    } catch (error) {
      tempArray[i] = false;
    }
  }

  //tempArray = array.forEach(pureBool(element));

  if (tempArray.includes(true) && tempArray.includes(false)) {
    bool = false;
  } else if (tempArray.includes(true)) {
    bool = true;
  } else {
    bool = false;
  }
 ...
 *   values include: none, nay, cc-by, cc-by-nc, cc-by-sa, cc-by-nd, cc-by-nc-sa, cc-by-nc-nd
 *
...
function buildUrl(query, order, count, license) {
  ...
  if (
    typeof order !== 'string' ||
    (order !== 'asc' && order !== 'desc' && order !== 'descending' && order !== 'ascending')
  ) {
    throw new Error('Invalid order.');
  }

  if (typeof count !== 'number' || count > 30 || count < 1) {
  ...
  incodedQ = encodeURIComponent(query);
  incodedQ = incodedQ.replaceAll("'", '');

  if (order === 'asc' || order === 'ascending') {
    fixedOrder = 'ascending';
  } else {
    fixedOrder = 'descending';
  }

  const url = `https://api.inaturalist.org/v2/observations?query=${incodedQ}&count=${count}&order=${fixedOrder}&license=${license}`;
