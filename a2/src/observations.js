...
function observationsByPrivacy(data, privacy) {
  ...
  for (var i = 0; i < data.results.length; i++) {
    if (data.results[i].privacy !== null && privacy !== null) {
      if (privacy.toLowerCase() === data.results[i].privacy.toLowerCase()) {
        data.results[i].privacy = data.results[i].privacy.toLowerCase();
        emptyArray.push(data.results[i]);
      }
    } else if (privacy === null && data.results[i].privacy === null) {
      emptyArray.push(data.results[i]);
    }
  }
...
function transformObservation(original) {
  ...
  if (original.id !== undefined) {
    var theReturn = {
      ...
      geoCoords: [parseFloat(coords[1]), parseFloat(coords[0])],
...
function transformObservations(data) {
  ...
  data.results.forEach((element) => emptyObject.push(transformObservation(element)));
...
function transformObservations2(data) {
  ...
  emptyArray = data.results.map((element) => transformObservation(element));
...
function getObservationsById(data, ...ids) {
  ...
  if (ids.length === 1) {
    exists = data.results.find((element) => element.id === ids[0]);
    if (exists) {
      return exists;
    }
  ...
  ids.forEach((id) => {
    if (id !== undefined) {
      const match = data.results.find((results) => results.id === id);
      if (match) {
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  return data.results.filter((element) => {
    if (options.eq) {
      return element.positional_accuracy === options.eq;
    } else if (options.gte && options.lte) {
      return (
        element.positional_accuracy >= options.gte && element.positional_accuracy <= options.lte
      );
    } else if (options.gte) {
      return element.positional_accuracy >= options.gte;
    } else if (options.lte) {
      return element.positional_accuracy <= options.lte;
    } else {
      return true;
    }
  });
...
function getTaxonPhotos(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].taxon && data.results[i].taxon.default_photo) {
      photoDictionary.push({
        original: `https://static.inaturalist.org/photos/575727/original.jpg?${data.results[i].id}`,
        square: `https://static.inaturalist.org/photos/575727/square.jpg?${data.results[i].id}`,
        small: `https://static.inaturalist.org/photos/575727/small.jpg?${data.results[i].id}`,
        medium: `https://static.inaturalist.org/photos/575727}/medium.jpg?${data.results[i].id}`,
        large: `https://static.inaturalist.org/photos/575727}/large.jpg?${data.results[i].id}`
      });
    }
  }
...
function getUserStats(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].user) {
      returnValue.count++;
      returnValue.totals.observations += data.results[i].user.observations_count;
      returnValue.totals.journals += data.results[i].user.journal_posts_count;
      returnValue.totals.species += data.results[i].user.species_count;
    }
  }

  returnValue.averages.observations = returnValue.totals.observations / returnValue.count;
  returnValue.averages.journals = returnValue.totals.journals / returnValue.count;
  returnValue.averages.species = returnValue.totals.species / returnValue.count;
...
function extractTimeZones(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].observed_time_zone) {
      array.push(data.results[i].observed_time_zone);
    }
  }
...
function extractTimeZones2(data) {
  var timeZonesSet = new Set();

  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].observed_time_zone) {
      timeZonesSet.add(data.results[i].observed_time_zone);
    }
  }

  var uniqueTimeZones = Array.from(timeZonesSet);
